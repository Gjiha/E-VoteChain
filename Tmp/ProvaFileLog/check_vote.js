...
const checkIfAlreadyVoted = async (req, res, next) => {
	try {
		...
		if (!meetingId || !voteIndex) {
			console.log(formatLog(req, 400, "Dati della votazione mancanti (meetingId o voteIndex)."));
			return res
				.status(400)
		...
		try {
			...
			console.log(formatLog(req, 200, `Nessuno storico trovato per meetingId: ${meetingId}, voteIndex: ${voteIndex}. Prima votazione consentita.`));
			return next();
		}
		...
		if (
			...
			console.log(formatLog(req, 200, `Storico vuoto per meetingId: ${meetingId}, voteIndex: ${voteIndex}. Voto consentito.`));
			return next();
		}
		...
		for (const record of historyResult) {
			...
			if (hashPartecipante === userHash) {
				console.log(formatLog(req, 403, `Voto duplicato rilevato per utente (hash: ${userHash}) su meetingId: ${meetingId}, voteIndex: ${voteIndex}.`));
		...
		console.log(formatLog(req, 200, `Nessun voto precedente rilevato per utente (hash: ${userHash}). Voto consentito.`));
		next();
	} catch (error) {
		console.error(formatLog(req, 500, `Errore nel middleware checkIfAlreadyVoted: ${error.message}`));
